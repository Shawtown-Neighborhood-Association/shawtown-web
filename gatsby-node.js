...
exports.onCreateNode = ({node, getNode, actions}) => {
  ...
  if (node.internal.type === 'Mdx') {
    ...
    if (source === 'agendas' || source === 'minutes') {
      ...
      const slug = `/meetings/${source}/${startDate.getFullYear()}/${format(startDate, "MMMMMM")}/${format(startDate, "dd")}/${slugify(node.frontmatter.type)}/`
      createNodeField({
        node,
        name: 'slug',
        value: slug
  ...
  mdxPages.data.allMdx.edges.forEach(({node}) => {
    if (node.fields?.source === 'agendas' || node.fields?.source === 'minutes') {
      createPage({
        ...
        component: path.resolve(`./src/templates/Meeting${node.fields.source === 'agendas' ? 'Agenda' : 'Minutes'}.tsx`),
        context: {
